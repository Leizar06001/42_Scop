...
bool loadOBJ(
	...
	for( unsigned int i=0; i < triVertexIndices.size(); i++ ){
		...
		unsigned int vertexIndex = triVertexIndices[i];
		unsigned int uvIndex = triUvIndices[i];
		unsigned int normalIndex = triNormalIndices[i];
	...
	for( unsigned int i=0; i < quadVertexIndices.size(); i++ ){
		...
		unsigned int vertexIndex = quadVertexIndices[i];
		unsigned int uvIndex = quadUvIndices[i];
		unsigned int normalIndex = quadNormalIndices[i];
		
		// Get the atquadbutes thanks to the index
		glm::vec3 vertex = temp_vertices[ vertexIndex-1 ];
        glm::vec2 uv;
        glm::vec3 normal;
        if (uvIndex != -1) {
